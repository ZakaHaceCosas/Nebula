...
export default class RPS {
  ...
  private getWinner(choice1: RPSChoice, choice2: RPSChoice): number {
  ...
  async run(interaction: ChatInputCommandInteraction) {
    ...
    if (opponent) {
      ...
      collector.on("collect", async (i: ButtonInteraction) => {
        const choice = i.customId.split("_")[1] as RPSChoice;
        playerChoices.set(i.user.id, choice);
      ...
      collector.on("collect", async (i: ButtonInteraction) => {
        ...
        const winner = this.getWinner(playerChoice, botChoice);
