...
const getAutokickQuery = autokickDb.query(
  "SELECT * FROM autokick WHERE guild = ?1 AND user = ?2;"
);
...
export function getAutokickData(guildId: string, userId: string): TypeOfDefinition<typeof autokickTableDefinition> | null {
  const results = getAutokickQuery.all(guildId, userId) as TypeOfDefinition<typeof autokickTableDefinition>[];
  return results[0] || null;
}

export function setAutokickData(
  guildId: string,
  userId: string,
  delay: number
): void {
  const now = new Date().toISOString();
  if (getAutokickQuery.all(guildId, userId).length) {
    deleteAutokickQuery.run(guildId, userId);
  }
  insertAutokickQuery.run(guildId, userId, now, delay);
}

export function removeAutokickData(
  guildId: string,
  userId: string
): void {
  deleteAutokickQuery.run(guildId, userId);
...
export function getAllAutokicks(guildId: string): TypeOfDefinition<typeof autokickTableDefinition>[] {
...
export function setAutokick(guildId: string, userId: string, delay: number): void {
  const res = getActivityCheckQuery.all(guildId, userId) as TypeOfDefinition<typeof activityChecksTableDefinition>[];
  if (res.length) return;

  insertActivityCheckQuery.run(guildId, userId, delay);
}

export function getAutokick(guildId: string, userId: string): TypeOfDefinition<typeof activityChecksTableDefinition> | null {
  const res = getActivityCheckQuery.all(guildId, userId) as TypeOfDefinition<typeof activityChecksTableDefinition>[];
  return res[0] || null;
}
